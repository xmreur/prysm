...
class _ChatScreenState extends State<ChatScreen> {
  ...
  void _handleNewMessages(List<Map<String, dynamic>> rawMessages) async {
    ...
    try {
      final decrypted = await decryptMessagesDeferred(rawMessages, widget.keyManager);
  ...
  _chatService.sendTextMessage(text, replyToId: replyToId, messageId: messageId).then((sentId) {
    if (sentId == null && mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Message queued. Will send when peer is available.')),
      );
    }
  });
}
...
Future<void> _sendFile(Uint8List bytes, String fileName, String type) async {
  ...
  _chatService.sendFileMessage(bytes, fileName, type, replyToId: replyToId, messageId: messageId).then((sentId) {
    if (sentId == null && mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('File queued. Will send when peer is available.')),
      );
    }
  });
}
...
Future<void> _handleSendFile() async {
    ...
    final result = await Navigator.push(
      ...
      MaterialPageRoute(
        builder: (context) => ChatProfileScreen(
          ...
          onDeleteChat: () async {
            await MessagesDb.deleteMessagesBetween(widget.userId, widget.peerId);
            resetChatState();
            setState(() {
              _messages = InMemoryChatController();
              _chatKey = UniqueKey();
  ...
  if (isSentByMe) {
    if (message.seenAt != null) {
      // Double tick (read)
      tickWidget = Icon(Icons.done_all, size: 14, color: Colors.blue.shade700);
...
Widget textMessageBuilder(
  ...
  return IntrinsicWidth( // ✅ ADD THIS: Makes container fit content
    child: Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        mainAxisSize: MainAxisSize.min, // ✅ ADD THIS: Don't expand vertically
        children: [
          Text(
            message.text,
            style: TextStyle(
