...
class ChatService {
    ...
    final _newMessagesController = StreamController<List<Map<String, dynamic>>>.broadcast();
    final _messageStatusController = StreamController<MessageStatusUpdate>.broadcast();

    Stream<List<Map<String, dynamic>>> get onNewMessages => _newMessagesController.stream;
    Stream<MessageStatusUpdate> get onMessageStatus => _messageStatusController.stream;
    ...
    Future<String?> sendTextMessage(String text, {String? replyToId, String? messageId}) async {
    if (peerPublicKey == null) return null;

    final timestamp = DateTime.now().millisecondsSinceEpoch;
    final id = messageId ?? const Uuid().v4(); // ✅ Use provided ID or generate new
    ...
    if (success) {
        ...
        await _addToPendingQueue(id, encryptedForPeer, 'text', replyToId: replyToId);
        _processSendQueue();
    }
...
Future<String?> sendFileMessage(
    ...
    String fileName,
    String type,
    {String? replyToId, String? messageId} // ✅ Add messageId parameter
) async {
    if (peerPublicKey == null) return null;
    ...
    final success = await _sendOverTor(
        id, payload['peerPayload']!, type,
        fileName: fileName, fileSize: bytes.length, replyToId: replyToId,
    );
    ...
    Future<void> _fetchNewMessages() async {
        ...
        final newMessages = batch.where((msg) => _newestTimestamp == null || (msg['timestamp'] as int) > _newestTimestamp!).toList();

        if (newMessages.isEmpty) return;

        _newestTimestamp = newMessages.map((m) => m['timestamp'] as int).reduce(max);
            ...
            while (!_disposed) {
                ...
                for (final msg in pending) {
                    ...
                    if (success) {
                        ...
                        _messageStatusController.add(MessageStatusUpdate(msg['id'], 'sent'));
                    } else {
    ...
    Future<bool> _sendOverTor(
        String id, String encrypted, String type, {
        String? replyToId, String? fileName, int? fileSize
    }) async {
        final isLargeMedia = type == 'file' || type == 'image';
        final timeout = isLargeMedia ? const Duration(minutes: 5) : const Duration(seconds: 30);
    ...
    static Map<String, String> _encryptFileIsolate(Map<String, dynamic> params) {
        ...
        final peerEncryptedKey = RSAHelper.encryptBytesWithPublicKey(aesKey.bytes, peerPubKey);
        final selfEncryptedKey = RSAHelper.encryptBytesWithPublicKey(aesKey.bytes, selfPubKey);
