...
class BatterySaverPolicy {
  ...
  static int chatPollIdleSeconds([bool? saving]) => (saving ?? active) ? 15 : 5;

  static Duration syncTickBacklog([bool? saving]) => (saving ?? active)
  ...
  static Duration homeRefreshInterval([bool? saving]) => (saving ?? active)
      ? const Duration(seconds: 120)
      : const Duration(seconds: 30);

  static Duration torHealthInterval([bool? saving]) => (saving ?? active)
      ? const Duration(seconds: 60)
      : const Duration(seconds: 15);

  static Duration trayPollInterval([bool? saving]) => (saving ?? active)
      ? const Duration(seconds: 60)
      : const Duration(seconds: 15);

  static Duration peerStatusInterval([bool? saving]) => (saving ?? active)
      ? const Duration(seconds: 300)
      : const Duration(seconds: 90);

  static Duration presenceActivityTtl([bool? saving]) => (saving ?? active)
      ? const Duration(seconds: 120)
      : const Duration(seconds: 90);
  ...
  static const Duration wakeHintMinOfflineBeforeBroadcast = Duration(
    seconds: 30,
  );

  static Duration wakeHintStaggerMin([bool? saving]) => (saving ?? active)
      ? const Duration(milliseconds: 500)
      : const Duration(milliseconds: 200);

  static Duration wakeHintStaggerMax([bool? saving]) => (saving ?? active)
      ? const Duration(milliseconds: 800)
      : const Duration(milliseconds: 400);
}
